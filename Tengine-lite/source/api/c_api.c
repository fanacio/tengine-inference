...
context_t create_context(const char* context_name, int empty_context)
{
    ...
    // 其实本质上没有进行执行操作，只是赋予了一个函数而已，在后续的地方才进行了context->scheduler的使用
    // 那么主要的函数就是context->scheduler->prerun就是了
...
int prerun_graph_multithread(graph_t graph, struct options option)
{
    ...
    // for aclm
    ctx->device_options = (struct cpu_option*)malloc(sizeof(struct cpu_option));
    memcpy(ctx->device_options, ctx->default_options, sizeof(struct cpu_option));

    struct scheduler* scheduler = ctx->scheduler;
    // 应该是在此处进行了后端layer的生成，即prerun
