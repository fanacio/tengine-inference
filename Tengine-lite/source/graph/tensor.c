...
// 从ir_graph中增加一个tensor，从89行可以看出确实是会跟ir_graph->graph_layout保持一致的。
...
// 增加设置tensor.layout的函数
int set_ir_tensor_layout(ir_tensor_t* tensor, const int layout)
{

    if (layout == TENGINE_LAYOUT_NCHW)
    {
        tensor->layout = TENGINE_LAYOUT_NCHW;
    }
    else if (layout == TENGINE_LAYOUT_NHWC)
    {
        tensor->layout = TENGINE_LAYOUT_NHWC;
    }
    else
    {
        fprintf(stderr, "%d is not NHWC or NCHW\n", layout);
        return -1;
    }
    return 0;
}
