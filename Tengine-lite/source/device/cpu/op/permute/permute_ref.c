...
// for shoot-yolox
static void __hcw(const float* input, float* output, int hh, int ww, int cc, int wc, int hw)
{
    for (int h = 0; h < hh; ++h)
    {
        float* output_ptr = output + h * wc; // chw
        for (int c = 0; c < cc; ++c)
        {
            const float* input_ptr = input + c * hw + h * ww; // input hwc + wc
            for (int w = 0; w < ww; ++w)
            {
                
                // hw + w = input_ptr[c]
                output_ptr[c*ww + w] = input_ptr[w];
            }
        }
    }
}
...
static int ref_permute_fp32(const struct tensor* input_tensor, const struct tensor* output_tensor, const permute_param_t* param)
{
    ...
    else if (param->order0 == 0 && param->order1 == 2 && param->order2 == 1 && param->order3 == 3)
    {
        for (int ii = 0; ii < n; ++ii)
        {
            __hcw(input, output, h, w, c, wc, hw);
