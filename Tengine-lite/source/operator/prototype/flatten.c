...
static int infer_shape(struct node* node)
{
    ...
    int new_channel = 1;
    int new_end_axis = flatten_param->end_axis;
    if (new_end_axis == -1)
    {
        new_end_axis = input->dim_num - 1;
    }
    for (int i = flatten_param->axis; i <= new_end_axis && i < input->dim_num; i++)
    {
    ...
    dims[2] = 1; 
    dims[3] = 1;
    for (int j = 1; j <= input->dim_num - 1; j++)
    {
        if (j < flatten_param->axis)
        {
            dims[j] = input->dims[j];
        }
        else if (j == flatten_param->axis)
        {
            dims[j] = new_channel;
        }
        else
    ...
    // dims[2] = 1;
    // dims[3] = 1;
    if(flatten_param->axis == 2)
        output->layout = TENGINE_LAYOUT_NCHW;
    else
