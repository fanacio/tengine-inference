...
static int load_conv(ir_graph_t* graph, ir_node_t* node, std::vector<std::string>& tensor_name_map, list* options, int index, FILE* fp)
{
    ...
    {
        std::string leaky_name = "leaky_" + std::to_string(index);
        ir_node_t* leaky_node = create_ir_node(graph, leaky_name.c_str(), OP_RELU, OP_VERSION);
        relu_param* leaky_param = (relu_param*)leaky_node->op.param_mem;
        leaky_param->negative_slope = 0.1f;
        set_ir_node_input_tensor(leaky_node, 0, out_tensor);
        std::string leaky_tensor_name = leaky_name + "_0";
        ir_tensor_t* leaky_out_tensor = create_ir_tensor(graph, leaky_tensor_name.c_str(), TENGINE_DT_FP32);
        set_ir_tensor_shape(leaky_out_tensor, out_dims.data(), out_dims.size());
        set_ir_node_output_tensor(leaky_node, 0, leaky_out_tensor);

        // update the tensor name map;
        tensor_name_map[index] = leaky_tensor_name;
    }
    ...
    if (strcmp(activation_s, "relu") == 0)
    {
        std::string relu_name = "relu_" + std::to_string(index);
    ...
    if (strcmp(activation_s, "hsigmoid") == 0)
    {
        std::string hsigmoid_name = "hsigmoid_" + std::to_string(index);
        ir_node_t* hsigmoid_node = create_ir_node(graph, hsigmoid_name.c_str(), OP_HARDSIGMOID, OP_VERSION);
        hard_sigmoid_param* param = (hard_sigmoid_param*)hsigmoid_node->op.param_mem;
        param->alpha = 0.1667f;
        param->beta = 0.5f;
        set_ir_node_input_tensor(hsigmoid_node, 0, out_tensor);
        std::string hsigmoid_tensor_name = hsigmoid_name + "_0";
        ir_tensor_t* hsigmoid_out_tensor = create_ir_tensor(graph, hsigmoid_tensor_name.c_str(), TENGINE_DT_FP32);
        set_ir_tensor_shape(hsigmoid_out_tensor, out_dims.data(), out_dims.size());
        set_ir_node_output_tensor(hsigmoid_node, 0, hsigmoid_out_tensor);

        // update the tensor name map;
        tensor_name_map[index] = hsigmoid_tensor_name;
    }
    if (strcmp(activation_s, "logistic") == 0)
    {
        std::string logistic_name = "logistic_" + std::to_string(index);
        ir_node_t* logistic_node = create_ir_node(graph, logistic_name.c_str(), OP_SIGMOID, OP_VERSION);
        set_ir_node_input_tensor(logistic_node, 0, out_tensor);
        std::string logistic_tensor_name = logistic_name + "_0";
        ir_tensor_t* logistic_out_tensor = create_ir_tensor(graph, logistic_tensor_name.c_str(), TENGINE_DT_FP32);
        set_ir_tensor_shape(logistic_out_tensor, out_dims.data(), out_dims.size());
        set_ir_node_output_tensor(logistic_node, 0, logistic_out_tensor);

        // update the tensor name map;
        tensor_name_map[index] = logistic_tensor_name;
    }
    return 0;
}
...
static int load_scale(ir_graph_t* graph, ir_node_t* node, std::vector<std::string>& tensor_name_map, list* options, int index, FILE* fp)
{
    ir_tensor_t* tensor = find_tensor(graph, tensor_name_map[index - 1]);
    if (tensor == nullptr)
    {
    ...
    if(tensor->dims[2]>tensor1->dims[2])
    {
        out_dims.push_back(tensor->dims[2]);
        out_dims.push_back(tensor->dims[3]);
    }
    ...
    {
        out_dims.push_back(tensor1->dims[2]);
        out_dims.push_back(tensor1->dims[3]);
    }
...
static int load_avg_pooling(ir_graph_t* graph, ir_node_t* node, std::vector<std::string>& tensor_name_map, list* options, int index, FILE* fp)
{
    ir_tensor_t* tensor = find_tensor(graph, tensor_name_map[index - 1]);
    if (tensor == nullptr)
    {
    ...
    set_ir_node_input_tensor(node, 0, tensor);
    int stride = option_find_int(options, (char*)"stride", 1);
    int size_h = option_find_int(options, (char*)"size_h", stride);
    int size_w = option_find_int(options, (char*)"size_w", stride);
    int padding = option_find_int_quiet(options, (char*)"padding", 0);

    pool_param* param = (pool_param*)node->op.param_mem;
    param->kernel_h = size_h;
    param->kernel_w = size_w;
    ...
    int in_h = tensor->dims[2];
    int in_w = tensor->dims[3];
    int out_h = (in_h + padding - size_h) / stride + 1;
    int out_w = (in_w + padding - size_w) / stride + 1;
    out_dims.push_back(out_h);
    out_dims.push_back(out_w);
...
void darknet_serializer::register_op_load()
{
    ...
    op_load_map["[scale_channels]"] = std::pair<int, op_load_t>(OP_ELTWISE, load_scale);
    ...
    op_load_map["[avgpool]"] = std::pair<int, op_load_t>(OP_POOL, load_avg_pooling);
